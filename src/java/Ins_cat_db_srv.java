...
public class Ins_cat_db_srv  extends HttpServlet
{
	Connection con;
	...
	public void service(HttpServletRequest hreq , HttpServletResponse hresp) throws ServletException,IOException
	{
		...
		try{
			    prst = con.prepareStatement("insert into CATEGORY  values(?,?,?)");
				...
				if(Rec>=1)
				{
						hresp.sendRedirect("cat_inserted.html");
				}
				else
				{
				...
				out.println("<BR><BR><center><a href=\"categ_add.jsp\">Back To Category Administration</a></center></h2>");	;}
	}
}